...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  return {
    ...
    isWwcCheckExpired: isDateExpired(user.wwcCheck[0]?.expiryDate),
    isPoliceCheckExpired: isDateExpired(user.policeCheck[0]?.expiryDate),
    welcomeCallCompleted: user.welcomeCall !== null,
    references: user.references,
    inductionCompleted: user.induction !== null,
    policeCheckCompleted: user.policeCheck.length > 0,
    wwcCheckCompleted: user.wwcCheck.length > 0,
