/*
  ...
  - A unique constraint covering the columns `[studentId,chapterId,attendedOn]` on the table `MentorToStudentSession` will be added. If there are existing duplicate values, this will fail.
...
-- DropIndex
DROP INDEX `MentorToStudentSession_userId_studentId_chapterId_attendedOn_key` ON `MentorToStudentSession`;

-- CreateIndex
CREATE UNIQUE INDEX `MentorToStudentSession_studentId_chapterId_attendedOn_key` ON `MentorToStudentSession`(`studentId`, `chapterId`, `attendedOn`);
